import { meterSerialFromNode } from '../components/order-form/meterId'
...
describe('meterSerialFromNode', () => {
  it('forwards a real backend meter serial', () => {
    expect(meterSerialFromNode(node({}))).toBe(
      'a3f1c2d4-5b6e-4a7c-8d9e-0f1a2b3c4d5e'
    )
  })
  ...
  it('drops synthesized transformer nodes', () => {
    ...
    // meter row behind it.
    expect(
      meterSerialFromNode(node({ id: 'transformer-3', type: 'transformer' }))
    ).toBeUndefined()
