...
export function ReadingsList({
  ...
  const filteredReadings = useMemo(() => {
    return readings.filter((reading) => {
      // Status filter
      if (statusFilter === 'minted' && reading.mint_status !== 'minted')
        return false
      if (statusFilter === 'pending' && reading.mint_status === 'minted')
        return false
  ...
  return (
    <Card className="flex h-full flex-col">
      ...
      <CardContent className="relative flex min-h-0 flex-1 flex-col p-3 pt-0">
        {loading ? (
          ...
          <div className="flex h-full items-center justify-center text-muted-foreground">
            No readings found. Generate some data!
          </div>
        ) : (
          <div className="flex h-full flex-col gap-3">
            <div className="flex min-h-0 flex-1 flex-col rounded-md border">
              <div className="grid flex-none grid-cols-6 gap-3 border-b bg-muted/50 px-3 py-2 text-xs font-medium text-muted-foreground">
                <div>Time</div>
                ...
                {paginatedReadings.length === 0 ? (
                  ...
                  paginatedReadings.map((reading) => {
                    ...
                    return (
                      <div
                        key={reading.id}
                        className="grid grid-cols-6 gap-3 border-b px-3 py-2 text-sm last:border-0 hover:bg-muted/50"
                      >
                        <div className="flex items-center font-mono text-xs">
                          {format(new Date(reading.timestamp), 'HH:mm:ss')}
                        </div>
                        <div className="col-span-1 flex flex-col justify-center truncate">
                          <div
                            className="truncate text-xs font-medium"
                            title={meter?.location || 'Unknown Location'}
                          >
                          ...
                          >
                            {meter?.meter_type.replace('_', ' ') || 'Meter'} (
                        ...
                        <div className="flex items-center">
                          {reading.kwh > 0 ? (
                            <span className="flex items-center text-green-500">
                              <ArrowUpRight className="mr-1 h-3 w-3" /> Gen
                            </span>
                          ) : (
                            <span className="flex items-center text-orange-500">
                              <ArrowDownRight className="mr-1 h-3 w-3" /> Cons
                            </span>
                          )}
                        </div>
                          ...
                          {reading.mint_tx_signature ? (
                            <>
                              <span className="truncate">
                                {reading.mint_tx_signature.slice(0, 12)}...
                              </span>
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() =>
                                  onCopy(reading.mint_tx_signature!)
                                }
