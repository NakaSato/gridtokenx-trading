...
export function useTopology(): UseTopologyResult {
  ...
  const [stats, setStats] = useState<TopologyInfo>({
    nodeCount: 0,
    lineCount: 0,
    totalLoss: 0,
  })
  ...
  const loadNetwork = useCallback(
    (nodes: EnergyNode[], transfers: EnergyTransfer[]) => {
      const nodeIdSet = new Set(nodes.map((n) => n.id))
      const validTransfers = transfers.filter(
        (t) => nodeIdSet.has(t.from) && nodeIdSet.has(t.to)
      )

      try {
        topologyRef.current.setGraph(nodes, validTransfers)
        // Bail out when the counts are unchanged. Always allocating a new
        // stats object made every call a real state update, so any caller
        // whose node/transfer arrays changed identity per render drove an
        // infinite render loop ("Maximum update depth exceeded").
        setStats((prev) =>
          prev.nodeCount === nodes.length &&
          prev.lineCount === validTransfers.length &&
          prev.totalLoss === 0
            ? prev
            : {
  ...
  const findPath = useCallback(
    (fromId: string, toId: string): PathResult | null => {
      try {
