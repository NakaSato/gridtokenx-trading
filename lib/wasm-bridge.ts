...
export function calculateBezier(
    ...
    intensity: number, segments: number
): [number, number][] {
...
export function auctionAddOrder(id: number, price: number, amount: number, is_bid: boolean): void {
...
export function auctionCalculateClearingPrice(): { price: number, volume: number } {
...
export function generatePnLBatch(
    ...
    for (let i = 0; i < numPoints; i++) {
        ...
        const pnl = calculatePnL(price, strikePrice, premium, contractType, positionType);
        prices.push(price);
        pnlData.push(pnl);
    }
...
function thetaCalcJS(s: number, k: number, t: number, isCall: boolean): number {
    ...
    const d1 = (Math.log(s / k) + (R + Math.pow(SIGMA, 2) / 2) * t) / (SIGMA * Math.sqrt(t));
    const d2 = d1 - SIGMA * Math.sqrt(t);
    let thetaValue = (-s * normalPdf(d1) * SIGMA) / (2 * Math.sqrt(t)) - R * k * Math.exp(-R * t) * normalCdf(isCall ? d2 : -d2);
...
function rhoCalcJS(s: number, k: number, t: number, isCall: boolean): number {
    ...
    const d1 = (Math.log(s / k) + (R + Math.pow(SIGMA, 2) / 2) * t) / (SIGMA * Math.sqrt(t));
    const d2 = d1 - SIGMA * Math.sqrt(t);
    return (isCall ? 1 : -1) * k * t * Math.exp(-R * t) * normalCdf(isCall ? d2 : -d2) * 0.01;
}
...
export function hmacVerify(key: string, message: string, signature: Uint8Array): boolean {
    ...
    const sigHex = Array.from(signature)
        ...
        .join('');
    return wasm_crypto_verify(encoder.encode(key), encoder.encode(message), sigHex);
}
...
export async function createTransferProof(
    ...
    if (!isWasmLoaded()) throw new Error('WASM not loaded');
    return wasm_create_transfer_proof(BigInt(amount), BigInt(balance), senderBlinding, amountBlinding);
}
