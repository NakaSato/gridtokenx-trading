...
const FIXTURE: OrderPayload = {
    ...
    energyAmount: 5_000_000_000n, // 5 kWh @ 9 decimals
    pricePerKwh: 2_500_000n, // 2.50 @ 6 decimals
    side: SIDE_SELL,
    zoneId: 1,
...
describe('buildOrderMessage', () => {
    ...
    it('encodes a negative expiry as two-s complement, like Rust i64::to_le_bytes', () => {
        const m = buildOrderMessage({ ...FIXTURE, expiresAt: -1n })
        expect(hex(m.slice(69))).toBe('ffffffffffffffff')
    })

    it('places each field at the offset the on-chain parser expects', () => {
        const m = buildOrderMessage({ ...FIXTURE, energyAmount: 7n, pricePerKwh: 9n, zoneId: 3 })
...
describe('uuidToBytes', () => {
    it('rejects a malformed UUID rather than emitting an unverifiable payload', () => {
        ...
        expect(() => uuidToBytes('0102030405060708090a0b0c0d0e0f')).toThrow(/Invalid order UUID/)
    })
...
describe('signOrderPayload', () => {
    it('signs the canonical message and returns a Base58 signature', async () => {
        const signMessage = jest.fn(async (m: Uint8Array) => new Uint8Array(64).fill(0x01))
