...
export class CarbonApi {
    ...
    async getCarbonBalance(): Promise<ApiResponse<CarbonBalanceResponse>> {
        return apiRequest<CarbonBalanceResponse>('/api/v1/users/me/carbon/balance', {
            method: 'GET',
            token: this.getToken(),
        })
    }
    ...
    async getCarbonHistory(): Promise<ApiResponse<CarbonCredit[]>> {
        return apiRequest<CarbonCredit[]>('/api/v1/users/me/carbon/history', {
            method: 'GET',
            token: this.getToken(),
        })
    }
    ...
    async getCarbonTransactions(): Promise<ApiResponse<CarbonTransaction[]>> {
        return apiRequest<CarbonTransaction[]>('/api/v1/users/me/carbon/transactions', {
            method: 'GET',
            token: this.getToken(),
        })
    }
    ...
    async transferCarbonCredits(data: {
        ...
        return apiRequest<{ transaction_id: string }>('/api/v1/users/me/carbon/transfer', {
            method: 'POST',
            body: data,
            token: this.getToken(),
        })
