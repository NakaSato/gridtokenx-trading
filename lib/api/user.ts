...
export class UserApi {
    ...
    async listWallets(): Promise<ApiResponse<UserWallet[]>> {
        return apiRequest<UserWallet[]>('/api/v1/wallets', {
            method: 'GET',
            token: this.getToken(),
        })
    }
    ...
    async removeWallet(walletId: string): Promise<ApiResponse<{ success: boolean }>> {
        return apiRequest<{ success: boolean }>(`/api/v1/wallets/${walletId}`, {
            method: 'DELETE',
            token: this.getToken(),
        })
    }
    ...
    async setPrimaryWallet(walletId: string): Promise<ApiResponse<{ success: boolean }>> {
        return apiRequest<{ success: boolean }>(`/api/v1/wallets/${walletId}/primary`, {
            method: 'PUT',
            token: this.getToken(),
        })
    }
    ...
    async getNotificationPreferences(): Promise<ApiResponse<NotificationPreferences>> {
        return apiRequest<NotificationPreferences>('/api/v1/me/notification-preferences', {
            method: 'GET',
            token: this.getToken(),
        })
    }
