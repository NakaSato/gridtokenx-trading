  ...
  searchSymbols: async (
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINT}/search?query=${encodeURIComponent(userInput)}`,
        {
          mode: 'cors',
          headers: {
    ...
    try {
      ...
      const response = await retryWithBackoff(async () => {
        const res = await fetch(
          `${API_ENDPOINT}/symbols?symbol=${encodeURIComponent(symbolName)}`,
          {
            mode: 'cors',
            headers: {
              Accept: 'application/json',
