 ...
 * @returns {any}
 */
export function create_unshield_proof(remaining, blinding) {
    const ptr0 = passArray8ToWasm0(blinding, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.create_unshield_proof(remaining, ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
function __wbg_get_imports() {
    const import0 = {
        ...
        __wbg_values_b16abac5653e529c: function(arg0) {
...
const WasmElGamalKeypairFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_wasmelgamalkeypair_free(ptr, 1));
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('zk_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
