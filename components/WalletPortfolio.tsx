...
export default function WalletPortfolio() {
  ...
  return (
    <div className="flex w-full flex-col space-y-2">
      ...
      {/* Private Section (ZK) */}
      <div className="flex w-full flex-col rounded-sm border border-primary/30 bg-primary/5 p-4 pt-3">
        <div
          className="flex w-full cursor-pointer justify-between"
        ...
        >
          <div className="flex items-center space-x-[6px]">
            ...
            <span className="flex h-4 items-center rounded-[4px] bg-primary/10 px-[6px] py-[5px] text-[10px] uppercase tracking-tighter text-primary">
              ...
              <div className="flex w-full items-center justify-between">
                <div className="flex h-fit items-center space-x-[10px]">
                  <div className="relative">
                    ...
                    />
                    <div className="absolute -bottom-1 -right-1 rounded-full border border-background bg-primary p-0.5 shadow-sm">
                  ...
                  <div className="flex flex-col space-y-0.5">
                    <div className="flex items-center space-x-2">
                      <span className="h-4 text-xs font-medium text-foreground">
                        Confidential GridToken
                      </span>
                      {privateBalance?.isInitialized && (
                        <span className="rounded bg-primary/10 px-1 text-[10px] font-bold text-primary">
                          {showAmount && privateBalance.amount !== null
                            ? `${privateBalance.amount.toLocaleString()} GRX`
                            : '•••••'}
                        </span>
                      )}
                    </div>
                    <div className="mt-0.5 flex items-center space-x-2">
                      <span className="block text-[9px] font-bold uppercase tracking-widest text-secondary-foreground">
                        Energy Units
                      </span>
                      {privateBalance?.origin && (
                        <span
                          className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-[8px] font-black uppercase ring-1 ring-inset ${
                            privateBalance.origin === 'Solar'
                              ? 'bg-orange-500/10 text-orange-500 ring-orange-500/20'
                              : privateBalance.origin === 'Wind'
                                ? 'bg-blue-500/10 text-blue-500 ring-blue-500/20'
                                : 'bg-primary/10 text-primary ring-primary/20'
                          }`}
                        >
                          {privateBalance.origin === 'Solar' ? (
                            <Sun size={8} className="mr-1" />
                          ) : privateBalance.origin === 'Wind' ? (
                            <Wind size={8} className="mr-1" />
                          ) : (
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="h-4 max-w-[150px] truncate font-mono text-[10px] text-secondary-foreground">
                        {privateBalance?.isInitialized
                          ? showCommitment
                            ? `C: ${Buffer.from(privateBalance.commitment).toString('hex')}`
                            : 'COMMITMENT HIDDEN'
                      ...
                      {privateBalance?.isInitialized && (
                        <div className="flex items-center space-x-1">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              setShowCommitment(!showCommitment)
                            }}
                            title="Toggle Commitment View"
                          >
                            <span className="ml-1 rounded border px-0.5 text-[9px] font-bold">
                              {showCommitment ? 'HEX' : 'HID'}
                            </span>
                          </button>
                        </div>
                      )}
                ...
                {privateBalance?.isInitialized ? (
                  <div className="flex space-x-1">
                    <button
                      ...
                      disabled={isReadOnly}
                      className="bg-secondary/50 flex items-center rounded border border-primary/20 px-2 py-1.5 text-[9px] font-bold text-foreground transition-all hover:bg-secondary disabled:opacity-30"
                    >
                    ...
                    <button
                      ...
                      disabled={isReadOnly}
                      className="rounded bg-primary px-2 py-1.5 text-[9px] font-bold text-primary-foreground transition-all hover:shadow-lg hover:shadow-primary/20 disabled:opacity-30"
                    >
                      SHIELD MORE
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => setIsShieldModalOpen(true)}
                    className="rounded border border-primary/50 px-3 py-1.5 text-[10px] font-bold text-primary transition-all hover:bg-primary/5"
                  >
                    INITIALIZE
                  </button>
                )}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 border-t pt-8 md:grid-cols-2">
        <GovernanceDashboard />
        <ConfidentialLending />
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 border-t pt-8 md:grid-cols-2 lg:grid-cols-3">
        <IoTSettlement />
        <IdentityGuard />
        <BridgePortal />
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 border-t pt-8 md:grid-cols-3">
        <MyOffers />
      ...
      <ShieldModal
        open={isShieldModalOpen}
        onOpenChange={(open) => setIsShieldModalOpen(open)}
      />
      <UnshieldModal
        open={isUnshieldModalOpen}
        onOpenChange={(open) => setIsUnshieldModalOpen(open)}
      />
      <StealthLinkGenerator
        open={isStealthModalOpen}
        onOpenChange={(open) => setIsStealthModalOpen(open)}
      />
      <StakePrivateModal
        open={isStakeModalOpen}
        onOpenChange={(open) => setIsStakeModalOpen(open)}
      />
      <BatchShieldModal
