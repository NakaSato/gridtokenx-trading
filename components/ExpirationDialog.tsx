...
export function ExpirationDialog({
  ...
  const filteredExpirations = allExpirations.filter(
    (exp) =>
      exp.period.toLowerCase().includes(searchExpiration.toLowerCase()) ||
      format(exp.date, 'dd MMM')
        .toLowerCase()
        .includes(searchExpiration.toLowerCase())
  )
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <Input
              ...
              className="rounded-sm border-border py-2 pl-10 pr-2 placeholder:text-muted focus:border-primary"
            />
          </div>
          ...
          <div className="border-t pt-4">
            {!showCustomExpiration ? (
              <Button
                ...
                className="w-full border bg-inherit px-4 py-5 text-primary hover:border-primary"
              >
              ...
              <form
                onSubmit={handleCustomExpirationSubmit}
                className="space-y-4"
              >
                <div className="space-y-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant={'outline'}
                        className={cn(
                          'w-full justify-start border-border text-left font-normal hover:bg-inherit hover:text-secondary-foreground',
